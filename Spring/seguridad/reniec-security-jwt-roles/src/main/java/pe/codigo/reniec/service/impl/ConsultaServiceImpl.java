...
public class ConsultaServiceImpl implements ConsultaService {
    ...
    @Override
    public void invalidar(String dni) {
        // Borramos del cache (Redis) y de la base de datos. Si no existia en la
        // BD, simplemente no hacemos nada (evitamos lanzar excepcion).
        redisTemplate.delete(PREFIJO_CACHE + dni);
        personaRepository.findById(dni).ifPresent(personaRepository::delete);
        log.info("DNI {} invalidado (cache + bd) por un ADMIN", dni);
    }
